...
class SurfaceActor(Actor):
    ...
    def __init__(self, label, spacing=[1,1,1], origin=[0,0,0], preset='vertebrae', centered=True, alpha_scale=1.):
        ...
        if isinstance(label, np.ndarray):
            label = numpy_to_volume(label, spacing, origin, order='zyx')
...
class VertebraSurfaceActor(Actor):

    """ 
    Actor for vertebrae surface rendering
    Note that background index should be zero.
    """
